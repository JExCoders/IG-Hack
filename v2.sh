...
echo -e '''
      \e[1;37m               
      \e[1;37m        -={•MULTI BRUTEFORCE INSTAGRAM•}=-
      \e[1;37m *Tools Jangan Diperjual belikan !
   \e[1;37m[\e[1;31m Blog \e[1;31m:\e[1;34m https://blog.linuxploit.com\e[1;37m ]
...
# asking
echo -ne "${white}[ ${red}?${white} ] Choose Options ${red}:${green} " ; read ask_menu
...
if [[ ! -e account_success_crack.txt ]]; then
	echo -e "${white}\n[ ${red}!${white} ] Ups you don't get any account :("
else
...
echo -ne "${white}[ ${red}?${white} ] Wanna Try Again ? ${red}(${white} y/n${red} ) :${green} " ; read ask_again
if [[ $ask_again == "Y" || $ask_again == "y" ]]; then
	echo -e "${white}[ ${red}+${white} ] Okay good! lets try again.."
	bash v2.sh
elif [[ $ask_again == "N" || $ask_again == "n" ]]; then
	echo -e "${white}[ ${red}+${white} ] See You Muachh.. :D"
	rm *.tmp*
else
	rm *.tmp*
fi
